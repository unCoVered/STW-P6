...
import server.parser.datos.DiaUtils;
...
public class Dia extends Element
{
	 ...
	 * @param dia
	 */
	public Dia(server.parser.datos.Dia dia)
	{
		super(server.common.CodeNames.DIA_ATR);
		this.dia = dia;
		this.setAttribute(server.common.CodeNames.FECHA_ATR, dia.getFecha());
		...
		this.probPrecipitacion = new Element(server.common.CodeNames.PROB_PRECIPITACION);
		this.cotaNieveProv = new Element(server.common.CodeNames.COTA_NIEVE_PROV);
		this.estadoCielo = new Element(server.common.CodeNames.ESTADO_CIELO);
		this.viento = new Element(server.common.CodeNames.VIENTO);
		this.rachaMax = new Element(server.common.CodeNames.RACHA_MAX);
		this.temperatura = new Element(server.common.CodeNames.TEMPERATURA);
		this.sensacionTermica = new Element(server.common.CodeNames.SENS_TERMICA);
		this.humedadRelativa = new Element(server.common.CodeNames.HUMEDAD_RELATIVA);
		this.uvMax = new Element(server.common.CodeNames.UV_MAX);
	...
	private void rellenarPeriodos(Element element, List<String> periodos)
	{
		...
		{
			element.setAttribute(server.common.CodeNames.PERIODO_ATR, periodo);
			...
			case "viento":
			{
				try
				{
					...
					if (datosViento != null)
					{
						this.viento.addContent(new Element(server.common.CodeNames.DIRECCION_ATR).setText(datosViento[0]));
						this.viento.addContent(new Element(server.common.CodeNames.VELOCIDAD_ATR).setText(datosViento[1]));
					}
				} catch (Exception ex)
				{
			...
			default:
				System.out.println(server.common.CodeNames.NO_IMPLEMENTADO_ERROR);
			}
		}
	}
	...
	private void rellenaObjetos(Element objeto, List<String> horas)
	{
		...
		case "temperatura":
		{
			try
			{
				...
				{
					this.temperatura.addContent(new Element(server.common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getTemperatura().getMaxima())));
					this.temperatura.addContent(new Element(server.common.CodeNames.MINIMA_ATR)
					...
					if (dia.getTemperatura().getDato().size() != 0)
					{
						for (String hora : horas)
						{
							this.temperatura.addContent(
									new Element(server.common.CodeNames.DATO_ATR).setAttribute(server.common.CodeNames.HORA_ATR, hora)
											.setText(
		...
		case "sens_termica":
		{
			try
			{
				...
				{
					this.sensacionTermica.addContent(new Element(server.common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getSensacionTermica().getMaxima())));
					this.sensacionTermica.addContent(new Element(server.common.CodeNames.MINIMA_ATR)
		...
		case "humedad_relativa":
		{
			try
			{
				...
				{
					this.humedadRelativa.addContent(new Element(server.common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getHumedadRelativa().getMaxima())));
					this.humedadRelativa.addContent(new Element(server.common.CodeNames.MINIMA_ATR)
